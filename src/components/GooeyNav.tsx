...
const GooeyNav = ({
  ...
  // Track active index based on initialActiveIndex or find index of activeHref
  const [activeIndex, setActiveIndex] = useState(() => {
    if (activeHref) {
      const foundIdx = items.findIndex(item => item.href === activeHref);
      if (foundIdx !== -1) return foundIdx;
    }
    return initialActiveIndex;
  });

  // Sync state if activeHref changes externally
  useEffect(() => {
    if (activeHref) {
      const foundIdx = items.findIndex(item => item.href === activeHref);
